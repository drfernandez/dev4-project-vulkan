...
#pragma pack(pop)
	...
	public:
		...
		bool Parse(const char* h2bPath)
		{
			...
			std::ifstream file;
			char buffer[260] = { 0, };
			file.open(h2bPath, std::ios_base::in | std::ios_base::binary);
			...
			file.read(reinterpret_cast<char*>(vertices.data()), static_cast<unsigned int>(36) * vertexCount);
			indices.resize(indexCount);
			file.read(reinterpret_cast<char*>(indices.data()), static_cast<unsigned int>(4) * indexCount);
			...
			file.read(reinterpret_cast<char*>(batches.data()), static_cast<unsigned int>(8) * materialCount);
		...
		void Clear()
		{
			*reinterpret_cast<unsigned int*>(version) = 0;
			file_strings.clear();
			vertices.clear();
			indices.clear();
			materials.clear();
